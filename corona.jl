...
# ╔═╡ 333afdb5-a433-431f-8a40-7cd572ce81d8
md"""
## API Keys
- [공공데이터활용지원센터_보건복지부 코로나19 감염 현황](https://www.data.go.kr/data/15043376/openapi.do) API Key
    - $(@bind key감염현황 TextField((70, 1)))
"""

# ╔═╡ 70440cfd-c8a4-4824-9956-134d520e04b0
begin
	function parseData(xpath, from)
		element = findfirst(xpath, from)
		if isnothing(element)
			return missing
		else
			return parse(Int, nodecontent(element))
		end
	end
end
...
# ╔═╡ 1e254bec-d857-4f22-b6c0-d8e20945b10a
md"""
## 보건복지부 코로나19 감염 현황
시작년월일
$(@bind 감염현황시작년 NumberField(2019:2021, default=2021))
$(@bind 감염현황시작월 NumberField(1:12, default=1))
$(@bind 감염현황시작일 NumberField(1:31, default=1))\
종료년월일
$(@bind 감염현황종료년 NumberField(2019:2021, default=2021))
$(@bind 감염현황종료월 NumberField(1:12, default=9))
$(@bind 감염현황종료일 NumberField(1:31, default=25))
"""

# ╔═╡ cc6910c8-218b-47e4-b15e-56ee7b322099
begin
	# https://www.data.go.kr/data/15043376/openapi.do
	# REST (GET), XML
	
	감염현황 = let
		serviceKey = strip(key감염현황)
		numOfRows = 10 # 서버에서 무시함
		pageNo = 1     # 서버에서 무시함
		startCreateDt = @sprintf("%04d%02d%02d",
			감염현황시작년, 감염현황시작월, 감염현황시작일)
		endCreateDt = @sprintf("%04d%02d%02d",
			감염현황종료년, 감염현황종료월, 감염현황종료일)
		...
		r = HTTP.request("GET", baseUrl, query=params)
		xmlStr = String(r.body)
		xmlDoc = parsexml(xmlStr)

		resultCode = parseData("//resultCode", xmlDoc)
		resultMessage = nodecontent(findfirst("//resultMsg", xmlDoc))
		
		df = DataFrame()
		if resultCode == 0
			@debug """감염현황 API at $(Dates.format(now(), "yyyy-mm-dd HH:MM:SS"))"""
				
			for item in findall("//items/item", xmlDoc)
				dateStr = nodecontent(findfirst("./stateDt", item))
				timeStr = nodecontent(findfirst("./stateTime", item))
				datetime = DateTime("$dateStr $timeStr", dateformat"yyyymmdd HH:MM")
				
				# ID
				ID = parseData("./seq", item)
				# 누적확진자수
				decideCnt = parseData("./decideCnt", item)
				# 누적음성수
				resultNegCnt = parseData("./resutlNegCnt", item)
				# 누적검사완료수 = 누적확진자수 + 누적결과음성수
				accExamCompCnt = parseData("./accExamCompCnt", item)
				# 검사진행수
				examCnt = parseData("./examCnt", item)
				# 누적검사수 = 누적검사완료수 + 검사진행수
				accExamCnt = parseData("./accExamCnt", item)
				# 사망자수
				deathCnt = parseData("./deathCnt", item)
				# 격리해제수
				clearCnt = parseData("./clearCnt", item)
				# 치료중환자수
				careCnt = parseData("./careCnt", item)
				...
				allowmissing!(df, [:누적음성수, :누적검사완료수, :누적검사수, :치료중환자수])
			end
			
			sort!(df, [:기준시간])
			df[!, :확진자수] = [missing; diff(df.누적확진자수)]
			df[!, :음성수] = [missing; diff(df.누적음성수)]
			df[!, :검사완료수] = [missing; diff(df.누적검사완료수)]
			df[!, :검사수] = [missing; diff(df.누적검사수)]
		else
...
# ╔═╡ 5a13cc2c-e315-4166-a90a-f73a5a3bd1d4
@df 감염현황 plot(
	:기준시간,
	:확진자수,
	label=hcat(["확진자 수"]) # should be a row vector
)
